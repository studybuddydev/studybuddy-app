<template>
  <div :class="date.getDay() === 1 ? 'day-slot first-day' : 'day-slot'">

    <div  v-if="date.getDate() === new Date().getDate()" >
      <div class="now-ball bg-primary" :style="{ top: `${nowHeight}px` }"  />
      <div class="now-bar bg-primary" :style="{ top: `${nowHeight}px` }" />
    </div>

    <div class="grid-structure times" v-if="date.getDay() === 1">
    ...
    <div class="grid-structure timeslots">
      <div class="title day-number">
        ...
        <v-card class="deadline px-1 ma-1" v-for="d in deadlines" :color="d.color ?? 'secondary'">
      ...
      <div class="grid borders">
        <div v-for="h in hours" class="timeslot borders">
          <div v-for="m in minutes" @click="clickOnTimeSlot(h, m)"></div>
        </div>
      </div>
    </div>
    ...
    <div class="events">
      ...
      <v-menu
        ...
        <v-card width="450" elevation="10" class="pa-4">
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" variant="flat" @click="deleteEvent(e)"> <v-icon icon="mdi-delete"/></v-btn>
          </v-card-actions>
        </v-card>
